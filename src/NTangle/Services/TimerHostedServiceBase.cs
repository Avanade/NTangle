...
namespace NTangle.Services
{
    ...
    public abstract class TimerHostedServiceBase : IHostedService, IDisposable
    {
        ...
        public TimerHostedServiceBase(IServiceProvider serviceProvider, ILogger logger, IConfiguration? config = null)
        {
        ...
        private async Task ScopedExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            catch (Exception ex)
            {
                ...
                Logger.LogCritical(ex, $"{ServiceName} execution failure as a result of an unexpected exception: {ex.Message}");
                throw;
            }
        }
