...
`trackingStoredProcedure` | The `CDC` _tracking_ (Sidecar only) batch stored procedure name.<br/>&dagger; Defaults to `sp` (literal) + `Name` + `BatchTracking` (literal); e.g. `spNameBatchTracking`.
