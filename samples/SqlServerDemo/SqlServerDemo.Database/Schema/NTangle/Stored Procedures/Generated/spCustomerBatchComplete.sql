...
BEGIN
  ...
  BEGIN TRY
    ...
    MERGE INTO [NTangle].[VersionTracking] WITH (HOLDLOCK) AS [_vt]
      USING #versionTrackingList AS [_list] ON ([_vt].[Schema] = N'Legacy' AND [_vt].[Table] = N'Cust' AND [_vt].[Key] = [_list].[Key])
      ...
      WHEN NOT MATCHED BY TARGET
        THEN INSERT ([Schema], [Table], [Key], [Hash], [Timestamp], [BatchTrackingId])
          VALUES (N'Legacy', N'Cust', [_list].[Key], [_list].[Hash], @Timestamp, @BatchTrackingId2);
