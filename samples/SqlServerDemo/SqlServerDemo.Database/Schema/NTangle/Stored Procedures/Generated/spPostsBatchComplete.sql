...
BEGIN
  ...
  BEGIN TRY
    ...
    MERGE INTO [NTangle].[VersionTracking] WITH (HOLDLOCK) AS [_ct]
      USING @VersionTrackingList AS [_list] ON ([_ct].[Schema] = 'Legacy' AND [_ct].[Table] = 'Posts' AND [_ct].[Key] = [_list].[Key])
      ...
      WHEN NOT MATCHED BY TARGET
        THEN INSERT ([Schema], [Table], [Key], [Hash], [BatchTrackingId])
          VALUES ('Legacy', 'Posts', [_list].[Key], [_list].[Hash], @BatchTrackingId);
